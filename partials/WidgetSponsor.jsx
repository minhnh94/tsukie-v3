...
function WidgetSponsor({ imageRotationClass, title, pjName, pjDesc, link, img }) {
  return (
    <a className="block rounded-lg border border-slate-200 dark:border-slate-800 dark:bg-gradient-to-t dark:from-slate-800 dark:to-slate-800/30 odd:rotate-1 even:-rotate-1 p-5"
       href={ link }
       target="_blank"
    ...
    >
      ...
      <div className="font-aspekta font-[650] mb-1">{ pjName }</div>
      <p className="text-sm text-slate-500 dark:text-slate-400 mb-3">{ pjDesc }</p>
      <div className="text-center">
        <img className={ `inline-flex rounded-lg shadow-lg ${ imageRotationClass }` } src={ img } alt="WallCal"/>
      </div>
    </a>
  )
}
